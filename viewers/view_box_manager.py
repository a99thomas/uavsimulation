...
from viewers.mav_box_viewer import MavBoxViewer
from viewers.mav_path_viewer import MAVAndPathViewer
from viewers.mav_box_waypoint_viewer import MAVAndWaypointViewer
...
class ViewManager:
        ...
        if self.animation_flag or self.data_plot_flag or self.sensor_plot_flag: 
            ...
            if self.animation_flag:
                if self.map_flag:
                    ...
                    self.mav_view = MavBoxViewer(app=self.app)  
    ...
    def update(self,
               ...
               path: MsgPath=None,
               delivery_zone: MsgPath=None,
               waypoints: MsgWaypoints=None,
               map: MsgWorldMap=None,
               box_state: MsgState=None,
               ):
        if self.animation_flag: 
            if self.map_flag is True:
                ...
                self.mav_view.update(true_state, path, waypoints, box_state, delivery_zone)
        ...
        if sim_time >= self.viz_update_time + SIM.viz_time:
            self.animation_flag = True
        else:
            self.animation_flag = False
        if self.animation_flag or self.data_plot_flag or self.sensor_plot_flag: 
            self.app.processEvents()
            self.viz_update_time = sim_time
        if self.video_flag is True: 

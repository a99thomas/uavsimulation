...
class MavDynamics:
    ...
    def _f(self, state, forces_moments):
        ...
        # print(phi,theta,psi)
        # print(theta)
        # pos_dot = np.array([[cos(theta) * cos(psi), sin(phi)*sin(theta)*cos(psi) - cos(phi) * sin(psi), cos(phi)*sin(theta)*cos(psi) + sin(phi)*sin(psi)],
        #                 [cos(theta)*sin(psi), sin(phi)*sin(theta)*sin(psi) + cos(phi)*cos(psi), cos(phi)*sin(theta)*sin(psi) - sin(phi)*cos(psi)],
        #                 [-sin(theta), sin(phi)*cos(theta), cos(phi)*cos(theta)]]) @ np.array([[u], [v], [w]])
        ...
        # Position Dynamics
        u_dot = np.array([[r*v - q*w],
                          [p*w - r*u],
                          [q*u - p*v]]) + (np.array([[fx],[fy],[fz]])/MAV.mass)
        
        ## rotational kinematics
        # e0_dot = np.array([[1, sin(phi) * tan(theta), cos(phi) * tan(theta)],
        #                    [0, cos(phi), -sin(phi)],
        #                    [0, sin(phi)/cos(theta), cos(phi)/cos(theta)]]) @ np.array([[p],[q],[r]])
        # print(e0_dot[0])
        e0_dot = 0.5 * np.array([[0, -p, -q, -r],
                                 [p, 0, r, -q],
                                 [q, -r, 0, p],
        ...
        x_dot = np.array([pos_dot[0], pos_dot[1], pos_dot[2], u_dot[0], u_dot[1], u_dot[2], e0_dot[0], e0_dot[1], e0_dot[2], e0_dot[3], p_dot[0], p_dot[1], p_dot[2]])
        # time.sleep(1)
        return x_dot
    ...
    def _update_true_state(self):
        ...
        self.true_state.Va = 0 #np.linalg.norm(self._state[3:5])

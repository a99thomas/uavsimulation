...
while sim_time < SIM.end_time:
    ...
    if path_manager.manager_requests_waypoints is True:
        ...
        viewers.update_planning_tree(
               ...
               radius=PLAN.R_min)
    # if path_manager.rewind is True:

    #     path_planner = PathPlanner(type=planner_type)
    #     waypoints = path_planner.update(world_map, estimated_state, PLAN.R_min)
    #     # start_pose = np.array([[estimated_state.north], [estimated_state.east], [-100]])
    #     # path_planner.rrt_straight_line.update(start_pose, end_pose=np.array([[0,0,-100]]).T, Va= 25., world_map=world_map, radius=PLAN.R_min)
    #     waypoints.ned = waypoints.ned[:, ::-1]
    #     # waypoints = waypoints.ned
    #     viewers.update_planning_tree(
    #            waypoints=waypoints,
    #            map=world_map,
    #            waypoints_not_smooth=path_planner.waypoints_not_smooth,
